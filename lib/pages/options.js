...
PageController.prototype = {
    ...
    _bindEvents: function () {
        ...
        $('#removeCustomFilter').on('click', function(e){
            ...
            $($('#userFiltersBox').val()).each(function(k, filter){
                f = filter;
                contentPage.sendMessage({type: 'removeUserRule', ruleText: filter});
            });
    ...
    _renderWhiteListFilter: function (whiteListFilters) {
        whiteListFilters.forEach(function(filter){
            $('#excludedDomainsBox').append($('<option>', {text: filter}));
        });
        /*
    ...
    _renderUserFilter: function (userFilters) {
        userFilters.forEach(function(filter){
            $('#userFiltersBox').append($('<option>', {text: filter}));
        });
        /*
        var saveCallback = function (item) {
            if (item.isNew) {
    ...
    _renderAntiBannerFilters: function () {
        contentPage.sendMessage({type: 'getAntiBannerFiltersForOptionsPage'}, function (response) {
            ...
            $('.subscriptionRemoveButton').on('click', function(){
                ...
                contentPage.sendMessage({type: 'disableAntiBannerFilter', remove: true, filterId: filterId});
                $(this).parent().remove();
            });
        }.bind(this));
    },
